...
Sobreposição: o filho A, **só no teste**, após o `FOR UPDATE` da aplicação no recurso indicado em `segurar_apos`, grava `a-segurou.json` e espera `liberar.json` com a transação ainda aberta. O filho B registra `DB::beforeExecuting` e grava `b-atingiu-for-update.json` **antes** de enviar o `FOR UPDATE` correspondente. Esse arquivo é o sinal de que B chegou à operação crítica com a transação de A ainda aberta. O pai só escreve `liberar.json` depois disso e **sem** `b-passou-for-update.json` (listener que só dispara quando esse `FOR UPDATE` de B **termina**). Se B só demora para bootar, `b-atingiu-for-update.json` não aparece e o caso estoura por timeout. Se B não espera o lock, `b-passou-for-update.json` surge antes da liberação e o caso falha. Depois da liberação, B pode concluir o `FOR UPDATE` ou, num deadlock com repetição, retornar o pedido já gravado sem um segundo `FOR UPDATE`; o que importa é ter despachado o primeiro enquanto A retinha o recurso. O usuário `ecommerce_e2e` não tem `PROCESS` nem leitura de `performance_schema.data_lock_waits`.
