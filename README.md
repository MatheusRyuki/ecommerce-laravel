...
A suíte `phpunit.concorrencia.xml` usa o MySQL InnoDB `ecommerce_e2e` (usuário `ecommerce_e2e`). Cada caso dispara dois processos PHP independentes, cada um com a própria conexão. A primeira transação permanece aberta após o `FOR UPDATE` da aplicação. No ponto da consulta crítica do segundo processo, o teste abre uma **conexão descartável** e executa o mesmo SQL com `FOR UPDATE NOWAIT`. Só o erro MySQL **3572** comprova o conflito; sucesso ou outro código recusam a prova. Marcadores anteriores ao envio da consulta, sozinhos, não liberam a primeira transação. A sondagem não observa a fila de espera da consulta normal da aplicação. Depois da prova, a aplicação segue com o `FOR UPDATE` usual. Um controle negativo confirma que a sonda sem bloqueio não devolve 3572.
