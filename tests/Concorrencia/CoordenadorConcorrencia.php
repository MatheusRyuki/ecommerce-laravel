...
final class CoordenadorConcorrencia
{
    ...
    public function executar(array $payloadA, array $payloadB): array
    {
        ...
        while ((microtime(true) - $inicioEspera) < 0.4) {
        ...
        $terminouAntes = is_file($this->dir.'/b-resultado.json');
        ...
        if ($terminouAntes) {
            throw new RuntimeException('O segundo processo concluiu enquanto a primeira transação ainda estava aberta; a disputa não foi observada.');
        }
        ...
        while (! is_file($caminho)) {
            foreach ($this->processos as $indice => $processo) {
                ...
                if (($status['running'] ?? true) === false && ! is_file($caminho)) {
                    $papel = $indice === 0 ? 'a' : 'b';
                    $erro = is_file($this->dir.'/'.$papel.'.err') ? (string) file_get_contents($this->dir.'/'.$papel.'.err') : '';
                    throw new RuntimeException('O processo '.$papel.' encerrou antes de '.$nome.'. '.$erro);
                }
            }
